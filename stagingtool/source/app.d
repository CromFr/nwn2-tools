...
int main(string[] args)
{
	...
	auto res = getopt(args, config.passThrough,
		...
		"ignore", "File pattern to exclude from downloadable files, e.g. '*-dev.hak'. Can be provided multiple times.", &ignoreGlob,
		"list", "Output the list of downloadable content to a separate file, similarily to the 'files.txt' generated by the toolset. '-' to print to stdout.", &listFilePath,
	...
	bool isFileIgnored(in string fileName){
		foreach(glob ; ignoreGlob)
			if(fileName.globMatch(glob))
				return true;
		return false;
	}
	...
	foreach(resPath ; resPaths){
		foreach(file ; resPath.dirEntries(SpanMode.shallow)){
			if(file.extension.toLower in extMap && !isFileIgnored(file.baseName)){
				resourceFiles ~= file;
			}
		}
	}
	...
	//Write file list
	if(listFilePath == "-")
		writeln(resources.map!(r => r.name).join("\n"));
	else if(listFilePath !is null)
		listFilePath.writeFile(resources.map!(r => r.name).join("\n"));
